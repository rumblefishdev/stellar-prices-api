  ...
  constructor(scope: Construct, id: string, props: ObservabilityStackProps) {
    ...
    this.currentPricesFreshnessAlarm = new cloudwatch.Alarm(
      ...
      'CurrentPricesFreshnessAlarm',
      {
        ...
        alarmDescription: `prices.current_prices has not been rewritten for over ${config.opsAlarms.currentPricesFreshnessSeconds}s: mv_current_prices, its sole writer (REFRESH EVERY 1 MINUTE), has stopped, is failing every refresh, or was dropped. GET /price still answers HTTP 200 with frozen prices and no other alarm sees it. This watches the WRITER, not its input: updated_at is the refresh start time, so stale candles still read fresh here (that is rollup-freshness-1m's job). A value near 315360000 s means the table is EMPTY: the last refresh returned no rows. Diagnose as the ClickHouse default user via system.view_refreshes (view = 'mv_current_prices': status, exception, last_success_time). Recover with SYSTEM START VIEW / SYSTEM REFRESH VIEW prices.mv_current_prices, or re-apply schema/current.sql if the view is gone (docs/runbooks/0072-current-prices-mv-rollout.md). Threshold is operator-tunable via config.opsAlarms.currentPricesFreshnessSeconds (task 0243).`,
        metric: new cloudwatch.Metric({
          namespace: 'Prices/Rollup',
          metricName: 'RollupLagSeconds',
          dimensionsMap: {
