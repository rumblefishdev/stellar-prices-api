...
**Asset Discovery Lambda** is designed as another writer, but its ledger scan
has never run on production (task 0256). **Read by:** `sdex-backfill` at run
...
| `prices.pool_registry`                                                             | `ReplacingMergeTree(updated_at)` | none                  | `(contract_id)`                                  | `sdex-backfill` CLI at run end; `pool-registry-seed` tool (task 0079); live Ledger Processor, new pools only (task 0291); `events-backfill --discover-pools` (task 0291); Asset Discovery Lambda (scan never run on prod, task 0256) | `sdex-backfill` at run start; live Ledger Processor at cold start (task 0078)                   |
