...
///
/// ⚠️ Unlike everything else in this crate this watches **correctness, not
/// liveness**: a wrong `close_usd` is fresh, present and invisible to every
/// other alarm here.
pub mod usd_sanity;

/// The two stored-data invariants of the `close_usd = 0` sentinel (ADR 0292,
/// task 0151): a candle with no price-forming fill carries no price, and a USD
/// close cannot exist without a close. See [`zero_invariants`] for why this is a
