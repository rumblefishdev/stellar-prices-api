...
//!
//! Shared by both ClickHouse sinks — the live Ledger Processor's
//! `ClickHouseSink` and the SDEX backfill CLI's `Sink` — so a fix in the retry
//! path fixes both (the same rationale that houses decode/bucket math here).
