...
//! | route | does |
//! | --- | --- |
//! | `GET`/`POST /api/key` | reveal — the lookup, plus the value. **Never creates.** A revoked key answers `404 key_revoked` with the date a new one can be issued |
//! | `POST /api/key/rework` | **revoke** (task 0191): deactivate the caller's key now, issue nothing. Session-authorized, deliberately — see below |
//!
...
pub const KEY_PATH: &str = "/api/key";
...
/// next period.
pub const REWORK_PATH: &str = "/api/key/rework";
...
mod tests {
    ...
    /// One segment under `/api/`, like sign-in: the currently-deployed
