...
impl AppConfig {
    ...
    /// stage name is `envName` and needs no lookup.
    pub async fn load_portal_keys(&mut self) -> Result<(), PortalKeysError> {
        if !self.portal_enabled {
            return Ok(());
        }
        let plan_id = free_plan_id().await?;
        let api_id = api_id().await?;
        let stage = api_stage()?;
        self.portal_keys = Some(
            crate::portal::keys::gateway::Gateway::from_ambient_config(plan_id, api_id, stage)
                .await,
        );
        Ok(())
    }
...
mod portal_load_tests {
    ...
    /// `PORTAL_OAUTH_SECRET_FILE`/`_NAME`, `PORTAL_FREE_PLAN_ID`/`_PARAM`,
    /// `PORTAL_API_ID`/`_PARAM`, `PORTAL_API_STAGE` and
