...
history:
  ...
  - date: 2026-09-15
    ...
    who: stkrolikiewicz
    note: >
      ⚠️ Renumbered 0246 → 0289 and pushed for the first time. The file was
      written 2026-09-01 08:32 as 0246 and sat UNTRACKED for two weeks, invisible
      to the board; meanwhile [[0168]] spawned its own 0246 the same day (the
      ohlcv peg-series BUG, since archived) and three tasks link that one. The
      lore rule this proves: pull, create, push — an ID picked without pulling
      collides. Content refreshed at the same time: the lock has ALREADY moved,
      from d61b359 to 31be5f74, when [[0277]] bumped stellar-xdr 28 — which is
...
[`Cargo.toml:31`](../../../Cargo.toml):

```toml
xdr-parser = { git = "https://github.com/rumblefishdev/soroban-block-explorer.git", branch = "develop" }
```

`Cargo.lock` resolved this to `d61b359f…` when this was written and to
`31be5f747abadb2e0cc0d2ae4f59702604519bd5` since [[0277]] (2026-09-15) — the
move was intended that time, which is the point: nothing distinguishes an
intended move from an accidental one.

Two things are true and are easy to conflate:

- **`cargo build` / `cargo check` will not move it.** The lockfile is committed
  and authoritative; CI gets exactly what the lock says.
...
One line in [`Cargo.toml:31`](../../../Cargo.toml):

```toml
xdr-parser = { git = "https://github.com/rumblefishdev/soroban-block-explorer.git", rev = "31be5f747abadb2e0cc0d2ae4f59702604519bd5" }
```
...
- [ ] `Cargo.toml` pins `xdr-parser` by `rev`, at the commit `Cargo.lock`
      already resolves to (`31be5f74` as of 2026-09-15 — re-read the lock first)
