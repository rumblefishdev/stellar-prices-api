...
status: completed
related_adr: []
related_tasks: ["0167", "0173", "0170", "0061", "0141", "0199", "0086", "0083", "0196", "0200", "0215", "0226", "0228", "0231"]
...
history:
  ...
  - date: 2026-08-28
    ...
    who: okarcz
    note: >
      CLOSED. Partition `197001` dropped on prod — 6,578 rows, both assets —
      and `min(timestamp)` for `reflector` now reads `2026-03-11 14:00:00`
      instead of `1970-01-21`, which is the query that handed [[0167]] a wrong
      coverage start date. The twin proof was re-run on BOTH assets first
      (**3,289/3,289** each; yesterday's 3,264/3,264 covered asset 3 only) and a
      TSV dump taken before the drop. Every surviving count moved upward
      afterwards, so nothing outside the partition was touched.
      ✅ The fix HOLDS at 24 h, which yesterday's check could not show: count
      unchanged to the row (6,578) and the slot frontier still
      `1970-01-21 16:37:21` across ~288 oracle-watcher runs. Minutes after a
      deploy, "nothing has changed yet" and "nothing will change" are the same
      reading; a day apart they are not.
      ✅ Asset 111 has **no rows under ANY `oracle_name`** — the open question
      the reflector-only census explicitly refused to generalise from. The USDT
      coverage gap handed to [[0173]] therefore stands on whole-table evidence,
      not on a census filter and not on [[0196]]'s purge.
      ✅ Retention settled from `cleanup-worker/src/lib.rs:33,63-73` rather than
      re-derived: `197001` was below any value the drop predicate can take, so it
      was eligible on every pass ever run — dropped daily, recreated within
      minutes, exactly as [[0086]] described. Post-fix it cannot be recreated, so
      [[0200]] can re-enable cleanup without the two colliding.
      ✅ No production code queries `min(timestamp)` on `oracle_prices`; the only
      reference was a stale doc comment on `usd_rate_population_it.rs` calling the
      defect intermittent and 0086 open. Corrected; the test stays as a guard on
      the snapshotter.
      ⚠️ Shipped as "rejected and LOGGED", not "rejected and ALARMED" — nothing
      reads the worker's `skipped` or `written == 0`. Recorded as unmet, not
      ticked, and owned by [[0231]].
      ⏳ Left open elsewhere on purpose: whether the two writers disagree on
      PRICE ([[0226]]) — n=2 post-fix, worthless until the poll path has a few
      dozen surviving rows.
---
...
- [x] The 6,372 existing rows are repaired or explicitly written off, with the
      decision recorded.
      → **Decision: DELETE. Executed 2026-08-28.** Partition `197001` dropped;
      6,578 rows gone, `min(timestamp)` for `reflector` now reads
      `2026-03-11 14:00:00` instead of `1970-01-21`. Both assets were re-proved
      at **3,289/3,289 exact price twins** immediately before the drop (yesterday's
      3,264/3,264 covered asset 3 only), and a TSV dump was taken first. Every
      surviving count moved upward, so nothing outside the partition was touched.
      See "EXECUTED 2026-08-28" below.
...
## ✅ EXECUTED 2026-08-28 — partition `197001` dropped, and `min(timestamp)` tells the truth

`ALTER TABLE prices.oracle_prices DROP PARTITION 197001`, run on prod as
`default` (`prices_writer` has no ALTER on `prices`). Instant metadata op — the
same mechanism [[0083]]'s cleanup worker uses (`cleanup-worker/src/lib.rs:73`),
not a row-by-row delete.

### The gate, re-proved before the drop

Yesterday's twin measurement covered **asset 3 only**, at 3,264. Both assets
were re-measured immediately before the drop:

| asset_id | corrupt rows | with an exact `Decimal(38,14)` price twin |
|---|---|---|
| 3 | 3,289 | **3,289 — 100%** |
| 4 | 3,289 | **3,289 — 100%** |

Asset 4 had never been twin-checked. It comes back equally clean, so the
"delete, do not repair" decision rests on both assets rather than one.

⚠️ A **TSV dump was taken first** (6,578 rows + header, verified as data and not
a captured error). The twin proof says the rows carry no information; the dump
costs nothing and makes that a statement with a file behind it rather than only
an argument.
...
| | before | after |
|---|---|---|
| rows in `197001` | 6,578 | **0** |
| `oracle_name`/`asset_id` pairs in table | 3 | **3** |
| `redstone` / 0 | 374,833 | 374,850 (+17) |
| `reflector` / 3 | 48,620 | 48,621 (+1) |
| `reflector` / 4 | 48,620 | 48,621 (+1) |
| **`min(timestamp)` for `reflector`** | **`1970-01-21 15:41:56`** | **`2026-03-11 14:00:00`** |

🔑 **Every surviving count moved UPWARD, which is the check that matters.** A
drop that took more than intended would show a pair missing or a count going
backwards; instead the 5-minute cadence kept writing throughout. Nothing outside
`197001` was touched.

🔑 **And the last line is the whole task.** `min(timestamp)` on `oracle_prices`
is the query that handed [[0167]] a wrong coverage start date. It now answers
`2026-03-11 14:00:00` — correct, from a table that is no longer accruing junk to
break it again.
