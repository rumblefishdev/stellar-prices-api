...
| `pf_trade_count = 0 ⇒ close = 0`, `close_usd > 0 ⇒ close > 0` and `pf_trade_count > 0 ⇒ close > 0` on stored rows, at the live tip (48 h of bucket time — a historical rewrite needs its own assertion) | `rollup-freshness-probe::zero_invariants` — metric `CandleZeroInvariantViolations`, alarm ladder `prices-{env}-zero-invariant-*`; ITs `the_zero_invariant_scan_counts_only_rows_that_break_an_invariant`, `a_repaired_candle_stops_counting_as_a_zero_invariant_violation` (`FINAL`), `a_violation_older_than_the_window_is_out_of_the_zero_invariant_scan`, `a_dust_minute_written_without_its_pf_column_is_a_zero_invariant_violation` |
